...
public class Test3
{
	public static void main(String ... args)
	{
		try
		{
			...
			TextNodeItem node2 = new TextNodeItem("RGB-out", new Connector(Direction.OUT, Connector.YELLOW)).setValueProvider(e->{
			...
			NodeBox nodeBox0 = new NodeBox("Color", slider1, slider2, slider3, slider0, node2).setSize(new Dimension(200,0));

			//

			SliderNodeItem slider10 = new SliderNodeItem("Alpha", 0, 1, 0.5).setOnChange((e,f)->{e.fireOnChange();});
			SliderNodeItem slider11 = new SliderNodeItem("Red", 0, 1, 0);
			SliderNodeItem slider12 = new SliderNodeItem("Green", 0, 1, 0.5);
			SliderNodeItem slider13 = new SliderNodeItem("Blue", 0, 1, 0.75);

			TextNodeItem node12 = new TextNodeItem("RGB-out", new Connector(Direction.OUT, Connector.YELLOW));

			NodeBox nodeBox4 = new NodeBox("Color", slider11, slider12, slider13, slider10, node12).setSize(new Dimension(200,0));
			...
			ImageNodeItem imageNodeItem = new ImageNodeItem("node10", image, 200, 200);
			NodeBox nodeBox1 = new NodeBox("Output", node0, slider4, imageNodeItem).setOnInputChange((e,f)->{
...
//				for (int y = 0; y < 200; y++) for (int x = 0; x < 200; x++) image.setRGB(x, y, ((int)(255*a)<<24)+((int)(255*r)<<16)+((int)(255*g)<<8)+(int)(255*b));
			...
			ColorChooserNodeItem node3 = new ColorChooserNodeItem("RGB-in", new Color(255,0,0), new Connector(Direction.IN, Connector.YELLOW));
			ColorChooserNodeItem node4 = new ColorChooserNodeItem("RGB-in", new Color(0,0,255), new Connector(Direction.IN, Connector.YELLOW));
			TextNodeItem node5 = new TextNodeItem("RGB-out", new Connector(Direction.OUT, Connector.YELLOW));
			SliderNodeItem slider6 = new SliderNodeItem("Mix", 0, 1, 0.5);
			NodeBox nodeBox3 = new NodeBox("Mix", node3, node4, slider6, node5).setSize(new Dimension(200,0));;
