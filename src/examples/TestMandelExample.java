...
public class TestMandelExample
{
	public static void main(String... args)
	{
		try
		{
			ProducerProperty mandelbrot = new ProducerProperty("Iterations")
			{
				@Override
				//public Object execute(@Param("coord") Vec2d coord, @Param("limit") int limit)
				public Object execute()
				{
					Vec2d coord = (Vec2d)getNode().getProperty("coord").execute();
					int limit = ((Number)getNode().getProperty("limit").execute()).intValue();

					double x0 = -2 + (2.47 * coord.x);
					double y0 = -1.12 + (2.24 * coord.y);
					int iteration = 0;
			...
			NodeModel model = new NodeModel()
				...
				.addNode(new Node("RenderOutput")
					...
					.addProperty(new ValueProperty("Vector").setId("coord").addConnector(IN, Styles.DefaultColors.PURPLE))
					.addProperty(o = new ImageProperty("", 200, 200))
					.addProperty(new ButtonProperty("Run").setIcon(Styles.DefaultIcons.RUN).setCommand("run"))
				)
				.addNode(new Node("Coordinate")
					.setTitleBackground(DefaultNodeColors.PURPLE)
					.setBounds(-400, 50, 150, 0)
					.addProperty(co = new ValueProperty("Vector").addConnector(OUT, Styles.DefaultColors.PURPLE))
				)
				.addConnection(0, 2, 1, 0)
				.addConnection(1, 5, 2, 0)
				.addConnection(3, 0, 2, 1)
				.addConnection(3, 0, 0, 1);

			NodeEditorPane editor = new NodeEditorPane(model)
				.center();

			editor.bind("run", property ->
			{
